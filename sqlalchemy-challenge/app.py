...
@app.route("/")
def welcome():
    percip_df = pd.read_sql("SELECT date, prcp FROM measurement", con=engine, columns=[["date"],["prcp"]])
    percip_df ["date"] = pd.to_datetime( percip_df["date"],format="%Y-%m-%d", errors="coerce")
    max_date = str(percip_df["date"].max().date()-dt.timedelta(days=1))
    pa_py_date = str(percip_df["date"].max().date()-dt.timedelta(days=366))
    return (
        ...
        f"<a href='/api/v1.0/temp-range/"+pa_py_date+"/"+max_date+"'>api/v1.0/temp-range/</a>start/end"
    )
